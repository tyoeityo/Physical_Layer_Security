...
%%%%求最大值及对应行数%%%%
    [Temp Location_pu_su]=max(M_pu_N_su_joint_sc_2);                       %M*R次搜索出的最大值,在M_pu_N_su_joint_sc的每列找出最大值Temp,Location_pu_sr记录最大值Temp所在行数
   
    %%%%确定max对应的最佳主用户%%%%
    Location_pu=zeros(1,L);
    for m=1:L
        if(mod(Location_pu_su(1,m),R)~=0)
            Location_pu(1,m)=fix(Location_pu_su(1,m)/R)+1;                 %max对应行数除以主用户数，若被整除，则第fix(Location_pu_sr(1,m)/R)+1个主用户为最佳主用户
        else
            Location_pu(1,m)=fix(Location_pu_su(1,m)/R);                   %max对应行数除以主用户数，若不被整除，则第fix(Location_pu_sr(1,m)/R)个主用户为最佳主用户
        end
    end
            
    %%%%确定max对应的最佳次用户%%%%
    Location_su=rem(Location_pu_su,R);                                     %max对应行数除以主用户数，若余数为零，则第R个次要中继为对应最佳次要中继
    Location_su(Location_su==0)=R;                                         %若余数不为零，则第rem(Location_pu_sr,M)
    
    %%%%注:第m次实验的最佳主用户：Location_pu(1,m)%%%%
    %%%%注:第m次实验的最佳次用户：Location_su(1,m)%%%%
    %%%%分别计算max对应的最佳主用户及最佳次用户的安全容量%%%%
    a=1;
    Temp_pu_sc=zeros(1,L);
    Temp_pu_sc_2=zeros(1,L);
    Temp_su_sc=zeros(1,L);
    for m=1:L
        Temp_pu_sc(1,a)=log(1+(Pp*abs(hir(Location_pu(1,m),m))^2)/(Ps(Location_su(1,m),m)*(abs(hjr(Location_su(1,m),m))^2)+N0))-log(1+(Pp*abs(hie(Location_pu(1,m),m))^2)/(Ps(Location_su(1,m),m)*(abs(hje(Location_su(1,m),m))^2)+N0));         %Location_pu(1,m)表示第m次实验时第Location_pu(1,m)个主用户为最佳主用户 
    ...
    Cs_M_pu_sc(k)=sum(Temp_pu_sc_2)/L;                                     %MER为某一值时，最佳主用户的安全容量均值
    Cs_N_su_sc(k)=sum(Temp_su_sc)/L;                                       %MER为某一值时，最佳次用户的安全容量均值
...
set(gcf,'color','white');
xlabel('MER(dB)')
ylabel('用户安全容量(bit/s/Hz)')
legend('location','NorthWest','用户安全容量之和（最佳中继）','用户安全容量之和（直接传输）','主用户安全容量（最佳中继）','主用户安全容量（直接传输）')
%legend('location','NorthWest','次用户安全容量（最佳中继）','次用户安全容量（直接传输）')